...
using QuadGK, LinearAlgebra, Dierckx
...
function discretizeKernel(basis::Basis, K::Function, xs::Array{Float64})
    #=
    ...
    for (m, x) in enumerate(xs)
        for (n, f) in enumerate(basis.basisFun)
            a, b = f.support
            r = quadgk(y -> K(y,x) * f.f(y), a, b, maxevals=10^7)
            Kmn[m][n] = r[0]
        end
    end
...
struct FourierBasis <: Basis
    ...
    function basisFunFourier(n::Int64, a::Float64, b::Float64)
        l::Float64 = (b - a) / 2.
        mid::Float64 = (a + b) / 2.
        func::Array{my_func} = [my_func(x -> 0.5, a, b)]
        for i = 1:n
            push!(func, my_func(x -> cos(n * pi * (x - mid) / l), a, b))
            push!(func, my_func(x -> sin(n * pi * (x - mid) / l), a, b))
        end
        return func
    end
...
function omega(basis::FourierBasis, deg::Int64)
    a, b = basis.a, basis.b
    delta = (b - a) / 2
    ...
    function basisFuncCubicSplines(a::Float64, b::Float64, knots::Array{Float64}, boundary::Union{Tuple, Nothing})
        ...
        function apply_cnd!(cnd::Union{String, Nothing}, side::String, basisFun::Array)
            ...
            if cnd == "dirichlet"
                ...
                elseif side == "r"
                    ...
                    error("CubicSpline: Unknown boundary side: " + side)
                end
            end
            error("CubicSpline: Unknown boundary condition: " + cnd)
        end
        ...
        if boundary != nothing
            ...
            elseif length(boundary) == 1
                ...
                error("Boundary conditions should be tuple")
            end
        end
        return basisFun
    end
...
function omega(basis::CubicSplineBasis, deg::Int32, equalize::Bool=False)
    #=
